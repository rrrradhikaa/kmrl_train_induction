...
def create_certificate(cert: schemas.FitnessCertificateCreate, db: Session = Depends(get_db)):
...
def get_certificate(certificate_id: str, db: Session = Depends(get_db)):
    cert = db.query(models.FitnessCertificate).filter(models.FitnessCertificate.certificate_id == certificate_id).first()
    if not cert:
